...
//
func printArea(width a : Float, height b : Float)  {
    print (a * b)
}

func printArea(radius a : Float) {
    print (a * a * 3.14)
}

printArea(width: 10.0, height: 10.0)
printArea(radius: 10.0)
...
func checkLoginStatus_if(_ logStatus : Bool, _ userName : String?){
    if logStatus {
...
// ---------- Mark : function guard문 사용 답안 작성란 ----------------
    func checkLoginStatus_guard(_ logStatus : Bool, _ userName : String?){
    guard logStatus else {
        print("로그인이 필요합니다.")
        return
    }
